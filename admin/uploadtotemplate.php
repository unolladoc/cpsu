...
function fileUpdload()
{
    ...
    $newbasename_filename_name = $basename_filename_name . "_TEMPLATE_" . $filerevision . "." . strtolower($fileextension);
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["filename"]["tmp_name"], $target_file_new)) {
            ...
            $sql = "INSERT INTO files values('$newid','$target_path_new','$newbasename_filename_name','$filedescription','$fileextension','$filepurpose','$filerevision',$fileuploader,'$fileorigin','$filedestination',0,CAST('$datenow' as datetime),0,3,'');";

            if ($conn->query($sql) === TRUE) {
                //copy($filepath, $destinationpath);
                //echo "New record created successfully";
                echo "templates.php?success=1";
                $sqll = "Insert into logs values(null, '" . $_SESSION['name'] . " uploaded " . $newbasename_filename_name . " in TEMPLATE',CAST('$datenow' as datetime), " . $_SESSION['id'] . ", 'UPLOAD TEMPLATE','$newid');";
